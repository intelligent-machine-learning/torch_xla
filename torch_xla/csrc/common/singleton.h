...
/// @brief This class abstracts the Singleton design pattern. It requires the real singleton
///        class to derive from this class, such as: class Derived : public Singleton<Derive>.
///        Furthermore, the Derived class should have a friend class Singleton<Derived> to allow
///        Singleton to call the private constructor.
///
/// @tparam T The derived class.
template <class T>
class Singleton {
 public:
    /// @brief Get the global singleton instance. Only the first call constructs the object.
    ///
    /// @return T*
    static T *GetInstance();

    /// @brief This is needed in ASAN. For a static singleton variable, we also do not delete it,
    ///        but the system will delete it before program exiting.
    ///        In ASAN UT case, this function should be called to avoid ASAN failure.
    ///
    static void Destroy();

 protected:
    /// @brief If the derived Singleton class does not define this function, `new T` will be used.
    ///        Otherwise, the ManualCreate function defined in the derived class will be used.
    ///
    /// @return T*
    ...
    ///
    /// @return true All Singleton objects except the SingletonManager should be true.
...
/// @brief Used to manage all singleton objects. We can destroy all singleton objects by DestroyAll.
///
...
T *Singleton<T>::GetInstance() {
    ...
    T *ptr = nullptr;
    {
        ...
        // If there is no T::ManualCreate defined, Singleton<T>::ManualCreate will be used.
...
T *Singleton<T>::ManualCreate() {
    // This function can be overwritten by writing a T::ManualCreate in the derived class T.
    return new T;
}
...
class SingletonResourceHolder : public SingletonResourceHolderBase {
 public:
    ...
    ///        Since the resource is a Singleton object, we use Singleton<T>::Destroy() to delete it.
    ///
    struct Deleter {
